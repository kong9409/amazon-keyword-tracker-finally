...
def parse_capture_form(form: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "lark": {
            ...
            "base_token": form.getfirst("base_token", "").strip(),
...
def first_form_field(form: Any, *names: str) -> Any | None:
    for name in names:
        if name in form:
            field = form[name]
...
class SimpleForm:
    ...
    def __contains__(self, name: str) -> bool:
        return name in self.files or name in self.values
...
def form_from_request(handler: BaseHTTPRequestHandler) -> SimpleForm:
    ...
    # JSON fallback for health checks or simple API clients.
